...
do
	...
	/usr/bin/time --append -f "$(date -u +%s),$run,%E" -o workloads/timing$TIMING_SUFFIX.csv workloads/run-job-workload-full-train.sh --dummy --repetitions $ITERATIONS $CACHE_OPT
done
